...
function block(at) {
  ...
  return [
    '════════ 이 글이 올라갈 날 ════════',
    /* ⚠️ 이 세 줄이 오늘의 운세에서 가장 중요한 지시다.
          띠를 먼저 정해놓고 근거를 나중에 붙이면 반드시 틀린다 —
          계미일에 용띠·원숭이띠가 나온 것이 딱 그 경우였다.
          그래서 덩어리 맨 앞, 날짜보다도 앞에 둔다. */
    '⚠️⚠️ **오늘의 운세는 일진을 먼저 검증한 뒤 글을 작성합니다.**',
    '   띠부터 고르고 근거를 나중에 붙이면 반드시 틀립니다.',
    '   **날짜 → 일진 → 지지 관계(합·충·형·해·파) → 띠 → 그제서야 글.**',
    '   이 순서를 건너뛰지 마세요. 아래 값이 그 검증을 끝낸 결과입니다.',
    '',
    ...
    '',
    /* ⚠️ 여기가 자꾸 어긋났다. 「일진을 그대로 쓰라」고만 했더니
          모델이 **첫 줄을 일진으로 바꿔** 버렸다. 틀의 첫 줄은 후킹이다 —
          「오늘은 을유일」로 시작하면 아무도 안 읽는다. */
    '⚠️ **일진은 틀이 일진을 적는 자리에만** 씁니다.',
    '   틀 첫 줄이 후킹이면 첫 줄은 후킹입니다. **일진으로 글을 시작하지 마세요.**',
    '   틀에 일진이 아예 없으면 일진을 적지 않아도 됩니다. 날짜만 맞으면 됩니다.',
    '',
