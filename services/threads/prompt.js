...
function chainBlock(chain) {
  ...
  return [
    ...
    /* ⚠️ 번호는 아예 안 붙인다. 예전엔 시스템이 붙였는데 끌 방법이 없어
          운세 글 끝에 「1/2」가 달려 나갔다. 모델도 적으면 안 된다. */
    '- **번호(1/2 · 2/2)는 붙이지 않습니다.** parts 에도 적지 마세요.',
    '',
  ].join('\n');
}
...
⚠️ **영어를 쓰지 마세요.** 로마자가 두 글자만 이어져도 못 나갑니다.
   Threads → 스레드 · DM → 디엠 · OK → 오케이
   다만 **vs 는 그대로 써도 됩니다** — 「인성 많은 사주 vs 식상 강한 사주」처럼.
...
**① 순서 틀은 그대로 따른다.** 이게 반응이 터진 이유다.
   ...
   **순서까지 피하려 들면 알맹이 없는 글이 된다.**
   ...
   ⚠️ **다만 아래에 「내 ○○ 틀」이 있으면 그것이 먼저다.** 이 자료의 순서보다
      본인이 쓰던 틀이 위다. 실제로 이 네 줄 순서 때문에 본인 인사글 틀이
      번호 목록으로 바뀌어 나갔다. 본인 틀이 있으면 그 틀을 채운다.
...
/**
 * 프롬프트 **맨 마지막**에 붙이는 못 박기.
 *
 * ⚠️ 본인 인사글 틀을 저장해뒀는데도 자꾸 딴 모양으로 나왔다.
 *    「사주상담가 운결담이라고 한다 / … 세 가지 알려준다 / 1. 2. 3.」처럼
 *    번호 목록이 되고 말투도 「~한다」로 바뀌었다.
 *
 *    프롬프트 가운데에 틀을 넣어두면 뒤에 오는 스물몇 줄짜리 규칙에
 *    묻힌다. 특히 벤치마크의 「인사글이면 인사 → 계기 → 차별점 → 댓글
 *    지정, 이 네 줄 순서를 지켜라」가 본인 틀을 이긴다.
 *
 *    그래서 **맨 마지막에 한 번 더** 짧게 박는다. 마지막에 읽은 것이
 *    제일 세게 남는다. 여기에는 틀 자체를 다시 통째로 넣지 않는다 —
 *    위에 이미 있으니 「그것을 채워라」만 못 박으면 된다.
 */
function lastWord(o) {
  const introSample = String((o.intro && o.intro.sample) || '').trim();
  const kindSample = String((o.sample && o.sample.text) || '').trim();
  const tpl = introSample || kindSample;
  if (!tpl) return '';

  const label = introSample ? '내 인사글' : ('내가 쓰던 ' + (o.sampleLabel || '이 종류') + ' 글');
  const NL2 = String.fromCharCode(10);

  return [
    '',
    '════════ 마지막으로 — 제일 중요합니다 ════════',
    '⚠️ 위에 **「' + label + '」**이 있습니다. 그것이 이 글의 **서식**입니다.',
    '   위의 어떤 규칙보다, 벤치마크의 순서 틀보다 **그 틀이 먼저입니다.**',
    '',
    '   · 줄 차례 · 줄 수 · 빈 줄 자리를 **그대로** 둡니다.',
    '   · **말투를 바꾸지 마세요.** 그 틀이 「~입니다」면 「~입니다」로,',
    '     「~함」이면 「~함」으로 끝까지 갑니다.',
    '   · 그 틀에 번호 목록이 없으면 **번호 목록을 만들지 마세요.**',
    '   · 그 틀에 없는 줄을 새로 만들어 넣지 마세요.',
    '',
    '   바꾸는 것은 **그 안에 들어가는 말**뿐입니다.',
    '   새로 쓰는 게 아닙니다. **서식을 채우는 것**입니다.',
    '',
    '── 지켜야 할 줄 차례 ──',
    shape.skeleton(tpl, '이 자리를 그대로 두세요'),
  ].join(NL2);
}
