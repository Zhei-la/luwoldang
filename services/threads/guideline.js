...
/* 경고형은 「이렇게 하면 된다」로 닫아야 한다.
   ⚠️ 예전엔 「이렇게」·「방향」 같은 몇 마디만 봤다. 그래서
      「가을까지만 버텨보세요」·「한 박자만 늦춰보세요」처럼
      멀쩡히 길을 알려주고 닫은 글이 「해결 문장 없음」으로 막혔다.
      한국어에서 길을 알려주는 자리는 대개 권유형 어미다. */
const SOLUTION = new RegExp([
  '세요', '십시오', '시길', '하시면',                 // 해보세요 · 마세요 · 하십시오
  '면 됩?니다', '면 돼', '면 충분',                   // 기다리면 됩니다
  '게 낫', '편이 낫', '것이 낫', '게 좋습니다',        // 옮기는 게 낫습니다
  '해야 합니다', '해야 할', '두셔야',
  '보셔야', '찾는 거', '방향', '정리하', '바꾸면', '이렇게',
  '권합니다', '추천', '해보', '해두',

  /* ⚠️ 여기까지는 전부 존댓말이다. 반말·음슴체로 쓰는 계정은
        「가을까지만 버텨봐」·「한 번 더 생각할 것」처럼 닫는데
        그게 하나도 안 걸려서, 경고형이 통째로 자동에서 막혔다.
        본인 말투가 반말이면 해결 문장도 반말로 나온다. */
  '[가-힣]\\s*봐',                                   // 버텨봐 · 남겨봐 · 늦춰봐
  '보자', '하자', '말자',                             // 기다려보자 · 서두르지 말자
  '면 됨', '면 좋', '면 나',                          // 조심하면 됨
  '게 좋', '게 나', '것이 좋',                        // 기다리는 게 좋음
...
/* 천간 열, 지지 열둘.
   지지는 한 글자라 그냥 넣으면 「사람」·「오늘」·「해」까지 걸린다.
   사주를 가리키는 자리에 쓰였을 때만 센다. */
const STEM = '갑을병정무기경신임계';
const BRANCH = '자축인묘진사오미신유술해';

/* 지지·간지를 가리킨 자리.

   ⚠️ 예전엔 천간(갑목·을목…)만 있고 지지가 통째로 빠져 있었다.
      그래서 「사주에서는 '진'자가 강할 때」처럼 지지를 딱 짚은 글이,
      그리고 **오늘의 운세가 쓰는 일진(계미일·무진일)이 전부**
      「사주 근거 없음」으로 막혀 자동으로 나가지 못했다. */
const QUOTE = '["\'‘’“”「」]';
/* 낱말이 여기서 끝났는지 — 조사가 붙거나 문장이 끊기면 끝난 것이다.
   이게 없으면 「자수성가」의 「자수」까지 지지로 센다. */
const JOSA = '(?=$|[\\s,.·…!?)\\]"\'’”」]|' +
  '가|이|은|는|을|를|의|로|와|과|도|만|에|나|랑|였|이라|라는|입니다|일수|처럼|보다)';
const BRANCH_RE = new RegExp([
  /* 계미일 · 무진월 · 갑자생 — 육십갑자에 날짜 말이 붙은 꼴 */
  '[' + STEM + '][' + BRANCH + ']\\s*(일|월|년|생|주|시)',
  /* 진일주 · 오월지 */
  '[' + BRANCH + ']\\s*(일주|월주|년주|시주|일지|월지|년지|시지)',
  /* 「'진'자가」 처럼 글자를 따옴표로 집어 보이는 꼴 */
  QUOTE + '\\s*[' + BRANCH + ']\\s*' + QUOTE + '?\\s*자',
  '[' + BRANCH + ']\\s*자(가|를|는|의|와|도|만|에)',
  /* 진토 · 인목 · 해수.
     뒤에 아무 글자나 붙는 것을 허용하면 「자수성가」까지 걸린다.
     그렇다고 한글을 통째로 막으면 「진토가」·「인목은」이 안 걸린다.
     조사와 문장 끝만 열어둔다. */
  '[' + BRANCH + '](토|목|화|금|수)' + JOSA,
  /* 지지에 진이 …  */
  '지지(에|가|는|의|를)?\\s*[' + BRANCH + ']',
  /* 일지·월지·시지·년지를 짚은 것만으로도 사주 이야기다 */
  '(일지|월지|시지|년지|일주|월주|시주|년주)',
].join('|'));
...
function pointsToSaju(text) {
  ...
  return ELEM_RE.test(t) || BRANCH_RE.test(t);
}
