...
- **오행 개수 하나만으로 성격이나 현실을 단정하지 마세요.**
  ❌ "금이 없어서 인내심이 부족합니다"
  ❌ "수가 부족해서 직관력이 약합니다"
  ❌ "금과 수가 없어서 재물 관리가 어렵습니다"
  → 오행은 출발점일 뿐입니다. 십성이 무엇인지, 어느 자리에 있는지,
    일간과 어떤 관계인지, 합충이 걸렸는지, 지금 운이 어느 쪽인지를 같이 보고 말합니다.
- **명식표에 없는 신살·귀인·합충을 만들어 내지 마세요.** 없으면 그 이야기를 안 하면 됩니다.
...
function myeongsikText(client, saju) {
  try {
    ...
    // 세운 10년 — 종합사주의 「연운」 장이 연도별로 풀려면 10년이 있어야 한다
    const out = buildMyeongsik(c2, { 세운년수: 10, 월운개월수: 12 });
...
1. **명식표에 없는 것을 지어낸 장** ★ 가장 먼저 보세요
   ...
   - 오행 개수·일간·격국·용신·대운 나이·연도가 명식표와 다른 장
