...
### 같은 조언을 장마다 그대로 쓰지 말고, 그 분야의 말로 바꾸세요
"남의 의견을 들으세요" 를 모든 장에 넣으면 다섯 장이 같은 글이 됩니다.
같은 뿌리에서 나온 조언이라도 그 장에서 실제로 할 수 있는 행동으로 바꿔 쓰세요.

  연애 → 상대에게 무엇을 기대하는지, 그것을 어떻게 말할지 정해 두기
  재물 → 한 달 지출 한도와 감당할 수 있는 위험 범위를 숫자로 적어 두기
  직업 → 역할과 결정권이 어디까지인지 분명한 자리를 고르기
  건강 → 몰두한 뒤 회복에 며칠이 필요한지 알아두고 그만큼 비워 두기

각 장의 마지막이 전부 "유연해지세요 / 경청하세요" 로 끝나면 그 리포트는 실패입니다.
...
${saju.gongmang && saju.gongmang.hits && saju.gongmang.hits.length ? `\n[공망]\n공망 지지: ${saju.gongmang.ko.join('·')}\n원국에서 걸린 자리: ${saju.gongmang.hits.map((h) => ({ year: '년지', month: '월지', day: '일지', hour: '시지' }[h])).join(', ')}\n→ 공망 자리의 글자는 "있어도 붕 떠서 힘을 제대로 못 쓰는" 상태로 봅니다. 그 자리의 기운이 늘 예상한 방식으로 드러나지는 않을 수 있다는 정도입니다.
⚠️ 공망 하나로 결론을 내지 마세요. 아래는 전부 과한 단정입니다.
   ✗ "재능을 발휘하지 못합니다" / "기회가 와도 잡지 못합니다"
   ✗ "인간관계에서 허전함과 실망이 반복됩니다" / "재물 기반이 불안합니다"
   → 공망은 보조 근거입니다. 다른 십성·합충·대운을 같이 보고 말하거나, 아예 짧게 넘어가세요.` : ''}
${(function () {
  // 간여지동 — 일간과 일지가 같은 오행
...
/* 12운성 뜻 — 지어내지 못하게 못 박아 둔다.
 *
 *   "병(病) 단계라 에너지가 왕성하게 솟구칩니다" 같은 글이 나갔다.
 *   왕성한 쪽은 건록·제왕이고 병은 기세를 조절해야 하는 단계다.
 *   단계마다 한 줄씩 붙여두면 반대로 읽는 사고가 줄어든다. */
const UNSEONG_MEAN = [
  '장생 — 막 시작해 뻗어나가는 자리',
  '목욕 — 아직 다듬는 중, 흔들림이 있는 자리',
  '관대 — 갖출 것을 갖추고 나서는 자리',
  '건록 — 제 몫을 온전히 해내는 자리',
  '제왕 — 기운이 가장 왕성한 자리',
  '쇠 — 정점을 지나 한풀 꺾이는 자리',
  '병 — 기세를 조절하고 관리해야 하는 자리 (왕성한 단계가 아닙니다)',
  '사 — 벌여둔 것을 정리하고 골라내는 자리 (창의력이 터지는 단계가 아닙니다)',
  '묘 — 갈무리해 안으로 넣어두는 자리',
  '절 — 끊기고 비워지는 자리',
  '태 — 새로 배태되는 자리',
  '양 — 조용히 길러지는 자리',
...
/** 자리와 십성을 짝지어 한 줄로 — "월지에 식신" 같은 오매핑을 막는다 */
function godIndex(saju) {
  const d = saju && saju.detail;
  if (!d) return '';
  const row = (key, gLabel, bLabel) => {
    const x = d[key];
    if (!x || !x.stem) return `${gLabel}·${bLabel}: (시간 모름)`;
    const g = x.stem.isDayMaster ? '나 자신' : x.stem.god;
    return `${gLabel} ${x.stem.ko}(${g}) · ${bLabel} ${x.branch.ko}(${x.branch.god}) · 12운성 ${x.unseong || '-'}`;
  };
  return [
    row('year', '년간', '년지'),
    row('month', '월간', '월지'),
...
/** 대운 — 지금 어디에 서 있고 다음이 무엇인지 */
function daewoonNow(chart) {
  const lines = String(chart || '').split('\n');
  const at = lines.findIndex((l) => l.indexOf('현재 대운') >= 0 && /^\s+\d+세/.test(l));
  if (at < 0) return '';
  const clean = (s) => String(s || '').replace(/←.*$/, '').trim();
  const cur = clean(lines[at]);
  const next = clean(lines[at + 1]);
  const after = clean(lines[at + 2]);
  const prev = at > 0 && /^\s+\d+세/.test(lines[at - 1]) ? clean(lines[at - 1]) : '';
  return [
    prev ? `직전 대운: ${prev}` : '',
    `현재 대운: ${cur}   ← 지금 여기입니다`,
    next ? `다음 대운: ${next}` : '',
...
/** 명식표를 프롬프트 블록으로 */
function chartBlock(chart, saju) {
  if (!chart) return '';
  const idx = godIndex(saju);
  const dw = daewoonNow(chart);
...
${chart}
${idx ? `
[자리와 십성 짝 — 짝지어 말할 때는 이 줄을 그대로 보고 쓰세요]
${idx}

⚠️ "월지에 식신" 처럼 위와 다르게 짝지으면 그 자리에서 틀립니다.
   자리를 말하려면 위 네 줄에서 찾아 쓰세요. 기억으로 쓰지 마세요.` : ''}
${dw ? `
[대운 — 지금 어디에 서 있는가]
${dw}

⚠️ "현재 운성" 이라고 쓸 때는 **현재 대운의 운성**만 씁니다. 다음 대운 것과 섞지 마세요.
⚠️ 대운의 십성은 위 괄호 안에 적힌 것뿐입니다.
   원국에 있는 십성을 그 대운이 데려온 것처럼 쓰지 마세요.
   (예: 대운이 편관·상관인데 "비견의 힘을 활용해" 라고 쓰면 틀립니다)
⚠️ 대운의 오행은 그 간지의 오행뿐입니다. 없는 오행을 보완해 준다고 쓰지 마세요.` : ''}

[12운성 단계 — 뜻을 지어내지 마세요]
${UNSEONG_MEAN.join(' / ')}
⚠️ 단계는 기운의 상태일 뿐 **길흉이 아닙니다.** 좋고 나쁨은 십성과 용희신이 정합니다.

[표기]
⚠️ 기둥 이름과 그 안의 글자를 같이 늘어놓지 마세요. 같은 말을 두 번 하는 것입니다.
   ❌ "시주에 자리한 기사와 사화는"   (기사 = 기토 + 사화)
   ⭕ "시주의 기토 정재와 사화 식신은" / "시주 기사에 자리한 정재와 식신은"

⚠️ 여기 없는 신살·귀인·합충형파해·격국은 **존재하지 않습니다.** 이름을 지어내지 마세요.
...
5-2. **근거를 잘못 쓴 장** ★ 여기를 특히 꼼꼼히 보세요
   - 자리와 십성을 잘못 짝지은 장 — [자리와 십성 짝] 과 대조하세요.
     ("월지에 식신" 인데 표에는 월지가 비견이고 식신은 월간인 경우)
   - 12운성 하나로 그 시기의 좋고 나쁨을 정한 장
     ("운성이 병이라 에너지가 왕성하게 솟구친다", "운성이 사라 창의력이 극대화된다")
   - 현재 대운의 운성과 다음 대운의 운성을 섞은 장
     ([대운 — 지금 어디에 서 있는가] 와 대조하세요)
   - 대운이 데려오지 않은 십성을 그 대운의 것처럼 쓴 장
     (대운이 편관·상관인데 "비견의 힘을 활용해" 라고 쓰는 식)
   - 대운이 없는 오행을 채워 준다고 쓴 장
     (토·화 대운을 두고 "금과 수의 부족을 보완한다" 고 쓰는 식)
   - 명식표의 [격국] 과 다른 격국 이름을 쓴 장
     (월지를 비견이라 해놓고 뒤에서 "식신격" 이라고 쓰는 식)
   - 공망 하나로 재능·기회·인간관계·재물을 단정한 장
     ("공망이라 재능을 발휘하지 못한다", "들어오는 만큼 나간다")
   - 기둥 이름과 그 안의 글자를 겹쳐 쓴 장 ("시주에 자리한 기사와 사화는")
   - 같은 요소(같은 십성·같은 운성·같은 신살)를 장마다 서로 다르게 푼 장
5-2-2. **근거 없이 심리를 묘사한 장** — 연애·성격 장에서 자주 나옵니다.
   "감정이 풍부하다", "상대 마음을 쉽게 읽는다", "작은 변화에도 민감하다" 처럼
   명식의 어느 자리를 보고 하는 말인지 댈 수 없는 문장만으로 채운 장
5-3. **선을 넘은 장**
...
7. **같은 결론이 리포트 곳곳에 되풀이된 장** — "자립심이 강하다",
   "고집을 조심해야 한다", "남의 말을 들어야 한다", "유연함이 필요하다" 가
   여러 장에 나오면 **가장 나중에 나온 장**을 고르세요 (처음 말한 장은 놔둡니다).
   여러 장의 마지막 문단이 전부 같은 조언으로 끝나는 것도 여기입니다.
...
async function generateChapter({ type, chapter, index, total, client, saju, partner, partnerSaju, openaiKey, model, allChapters, card, redoReason, done, chart }) {
  ...
  const info = sajuBlock(client, saju, partner, partnerSaju, type)
    + chartBlock(chartText, saju) + renderCard(card);
...
/* 간지 글자와 한자 숫자가 겹친다.
 *
 *   지지의 「사」와 「오」는 한자 숫자 4·5 이기도 하다.
 *   그래서 "병오 세운" 의 "오 세" 를 나이로 읽어 "병5세운" 이 되어 나갔다.
 *   ("정사 세운" → "정4세운", "기사 월운" → "기4월운" 도 같은 사고다)
 *
 *   간지는 언제나 천간 + 지지다. 앞 글자가 천간이면 그 뒤의 사·오는 숫자가 아니다.
 *   숫자로 바꾸는 자리마다 이 조건을 앞에 붙인다. */
const NOT_GANJI = '(?<![갑을병정무기경신임계])';
...
function fixKoreanNumbers(text) {
  ...
  // 한자수사 + 살/세 — 삼십살 → 30세
  //   "병오 세운" 의 "오 세" 를 잡지 않도록 앞 글자가 천간이면 건너뛴다
  t = t.replace(new RegExp(NOT_GANJI + '([일이삼사오육칠팔구십백]{1,4})\s*(살|세)(?![기대운])', 'g'), (m, ko, unit) => {
    const n = sinoToNum(ko);
    return (n && n >= 1 && n <= 120) ? n + unit : m;
  });

  // 월 — 팔월 → 8월 (연도 뒤에 오는 경우가 많다)
  //   "기사 월운" 의 "사 월" 을 잡지 않도록 앞 글자가 천간이면 건너뛴다
  t = t.replace(new RegExp(NOT_GANJI + '([일이삼사오육칠팔구십]{1,3})\s*월(?![급간운])', 'g'), (m, ko) => {
    const n = sinoToNum(ko);
...
/* 본문 후처리 — scripts/pdf-textcheck.js 가 회귀 검사에 쓴다.
   간지("병오 세운")를 숫자로 오인해 "병5세운" 으로 내보낸 적이 있다. */
module.exports.fixKoreanNumbers = fixKoreanNumbers;
