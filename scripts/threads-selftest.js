...
section('말투는 한 글에 하나로');

const SP = require(require('path').join(__dirname, '..', 'services', 'threads', 'speech'));
const BAD_MIX = ['안녕하세요 사주상담가 운결담 입니다', '',
  '오늘은 사주 촉이 제대로 와서 무료로 열어봅니다',
  '복채는 댓글에 스하리 한 번이면 충분해요', '',
  '가볍게 신청하고 싶은 분들',
  '댓글에 생년월일시ㆍ성별ㆍ요즘 제일 고민되는 거 하나 남겨줘'].join(LF);

t('섞인 글을 잡는다', SP.mixed(BAD_MIX).ok, false);
/* 어느 줄이 튀는지 짚어줘야 고칠 수 있다 */
t('튀는 줄을 짚어준다', SP.mixed(BAD_MIX).why.indexOf('남겨줘') > 0, true);
t('많은 쪽을 지킨다', SP.mixed(BAD_MIX).why.indexOf('이 글은 **존댓말**입니다') > 0, true);
t('존댓말 줄을 센다', SP.classify(BAD_MIX).polite.length, 3);
t('반말 줄을 센다', SP.classify(BAD_MIX).plain.length, 1);
/* ⚠️ 적은 쪽이 한 줄뿐인 것이 제일 흔하다. 마지막 줄만 반말인 경우다. */
t('한 줄이어도 잡는다', SP.classify(BAD_MIX).plain.length, 1);

/* ⚠️ 여기가 제일 무섭다 — 잘못 잡으면 멀쩡한 글이 계속 다시 만들어진다.
      요금만 나가고 글은 그대로다. 아래는 전부 통과해야 한다. */
const clean = {
  '순수 음슴체': ['갑목은 곧게 자람', '', '휘지 않아서 부러지기도 함', '', '한 박자 늦춰봐'],
  '순수 존댓말': ['경금 일간은 거절을 못 합니다', '', '그래서 일이 몰립니다', '', '한 박자 늦춰보세요'],
  '순수 해요체': ['오늘은 흐름이 좋아요', '', '천천히 가도 괜찮아요'],
  '운세 이모지': ['오늘 잘 갈리는 날', '', '9월 10일 오늘의 운세', '',
    '🍀 운 좋은 띠', '🐉 용띠', '🐰 토끼띠', '', '⚠️ 조심할 띠', '🐍 뱀띠'],
  '명사로 끝남': ['그게 제일 중요', '', '필요', '', '사람'],
  '마음·처음': ['사주 보는 게 처음', '', '제일 큰 건 마음'],
  '리스트형': ['말이 늦게 나오는 사람들이 있음', '',
    '1. 생각을 다 끝내고 말함', '2. 틀릴까 봐 한 번 더 봄', '', '먼저 반만 말해봐'],
  '일간별 리스트': ['같은 연락 씹힘이어도 일간 따라 반응이 완전 다름', '',
    '갑목- 답장 없으면 먼저 카톡 또 보냄', '을목- 속상해도 그냥 넘기고 티 안내', '',
    '본인 일간하고 맞는지 댓글에 남겨봐'],
};
Object.keys(clean).forEach((k) => {
  t('멀쩡한 글은 안 잡는다 — ' + k, SP.mixed(clean[k].join(LF)).ok, true);
});

/* 말투 하나 고르기 */
t('존댓말을 알아본다', SP.of(clean['순수 존댓말'].join(LF)), '존댓말');
t('반말을 알아본다', SP.of(clean['순수 음슴체'].join(LF)), '반말');
t('못 고르면 빈 값', SP.of(clean['명사로 끝남'].join(LF)), '');

/* 첫 댓글만 반말인 경우도 있다 */
t('첫 댓글까지 같이 본다',
  SP.mixedIn(['오늘은 무료로 열어봅니다'], '고민 하나 남겨줘').ok, false);
t('본문·댓글이 같으면 통과',
  SP.mixedIn(['오늘은 무료로 열어봅니다'], '고민 하나 남겨주세요').ok, true);

/* 프롬프트가 애초에 못 섞게 이른다 */
const spPrompt = P6.buildPrompt('무료사주 인사',
  { sample: { kind: 'intro', text: '안녕하세요 운결담입니다' + LF + LF + '오늘 무료로 열어봅니다' } });
t('프롬프트가 말투를 못 박는다', spPrompt.indexOf('존댓말과 반말을 섞지 마세요') > 0, true);
t('본보기 말투를 짚어준다', spPrompt.indexOf('이 계정은 **존댓말**을 씁니다') > 0, true);
t('본보기가 없으면 아무 쪽이나',
  P6.buildPrompt('재물운', {}).indexOf('어느 쪽이든 좋습니다') > 0, true);
t('첫 댓글도 맞추라고 한다', spPrompt.indexOf('첫 댓글도 본문과 같은 말투로') > 0, true);

/* ⚠️ 이 검사는 **발행을 막지 않는다.** 잘못 잡으면 멀쩡한 글이 통째로
      못 나간다. 만들 때 한 번 다시 시키는 데만 쓴다. */
/* 「쓰지 않는 말투」(금지어) 줄은 원래 있다. 말투 **섞임**을 재는 줄이
   생기면 안 된다는 뜻이다 — 그건 발행을 막는 자리다. */
t('지침 점검에는 안 넣는다',
  (checkPost({ postType: '정보형', form: 'single', topic: '재물운',
    parts: [BAD_MIX] }).rows || []).some((r) => r.label.indexOf('섞') >= 0), false);
t('섞여도 발행은 막지 않는다',
  checkPost({ postType: '정보형', form: 'single', topic: '무료사주 인사',
    parts: [BAD_MIX] }).passHard, true);

/* 만들 때 한 번 다시 시킨다 */
t('만들 때 다시 시킨다', pipeSrc3.indexOf('speech.mixedIn(p.parts, p.replyText)') > 0, true);
/* 더 섞여 왔으면 처음 것이 낫다 */
t('나아졌을 때만 바꾼다', pipeSrc3.indexOf('stillBad < mix.length') > 0, true);
