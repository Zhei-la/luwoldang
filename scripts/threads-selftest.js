...
/* ── 인사글은 설정해둔 틀 그대로 ──
   ⚠️ 설정에 인사글 틀을 넣어뒀는데도 자꾸 딴 모양으로 나왔다.
        ...
        1. 생년월일시  2. 성별  3. …
      번호 목록이 되고 말투도 「~한다」로 바뀌었다.
      벤치마크의 「인사글이면 인사 → 계기 → 차별점 → 댓글 지정」이
      본인 틀을 이긴 것이다. */
section('인사글은 설정해둔 틀 그대로');

const INTRO_TPL = ['안녕하세요 사주상담가 운결담 입니다', '',
  '오늘은 사주 촉이 제대로 와서 무료로 열어봅니다',
  '복채는 댓글에 스하리 한 번이면 충분해요', '',
  '가볍게 신청하고 싶은 분들',
  '댓글에 생년월일시ㆍ성별ㆍ요즘 제일 고민되는 거 하나 남겨줘'].join(LF);
const INTRO_BAD = ['사주상담가 운결담이라고 한다', '오늘 무료로 사주 신청 받고 있음',
  '신청할 때 꼭 적어야 할 것 세 가지 알려준다', '',
  '1. 생년월일시', '2. 성별', '3. 요즘 제일 답답한 고민 하나', '',
  '댓글에 이것만 달아주면 바로'].join(LF);
const INTRO_OK = ['안녕하세요 사주상담가 운결담 입니다', '',
  '오늘은 흐름이 좋아서 무료로 열어봅니다',
  '복채는 댓글 한 번이면 충분해요', '',
  '편하게 신청하고 싶은 분들',
  '댓글에 생년월일시ㆍ성별ㆍ요즘 고민 하나 남겨줘'].join(LF);

/* ⚠️ 예전엔 「입니다·습니다·합니다·됩니다」 넷만 셌다. 그래서
      「열어봅니다」가 안 잡혀 이 틀이 「말투 없음」으로 나왔고,
      말투가 안 잡히니 딴 말투로 나와도 그냥 통과했다. */
t('열어봅니다도 합니다체로 센다', DS.outline(INTRO_TPL).tone, '합니다체');
t('드립니다도 센다', DS.tone('무료로 봐드립니다'), '합니다체');
t('갑니다도 센다', DS.tone('오늘은 여기까지 갑니다'), '합니다체');

/* 틀에 없던 번호 목록이 생기는 것이 이 사고의 알맹이였다 */
t('번호 목록이 생기면 잡는다', DS.check(INTRO_TPL, INTRO_BAD).ok, false);
t('무엇이 생겼는지 말해준다',
  DS.check(INTRO_TPL, INTRO_BAD).why.indexOf('번호 목록') > 0, true);
t('번호 줄을 센다', DS.outline(INTRO_BAD).numbered, 3);
t('틀대로면 통과', DS.check(INTRO_TPL, INTRO_OK).ok, true);
/* 틀에 원래 번호가 있으면 번호를 써야 한다 — 붙잡으면 안 된다 */
const NUM_TPL = ['이런 사람들 있음', '', '1. 첫째', '2. 둘째', '3. 셋째'].join(LF);
t('원래 번호 틀은 안 잡는다',
  DS.check(NUM_TPL, ['저런 사람들 있음', '', '1. 하나', '2. 둘', '3. 셋'].join(LF)).ok, true);

/* 프롬프트가 맨 마지막에 다시 못 박는가 — 가운데에 두면 뒤 규칙에 묻힌다 */
const introP = P6.buildPrompt('무료사주 인사',
  { intro: { name: '운결담', career: '10년차', sample: INTRO_TPL } });
t('맨 마지막에 못 박는다', introP.indexOf('마지막으로 — 제일 중요합니다') > 0, true);
t('벤치마크보다 위라고 적는다',
  introP.indexOf('벤치마크의 순서 틀보다 **그 틀이 먼저입니다.**') > 0, true);
t('번호 목록 금지를 적는다', introP.indexOf('번호 목록을 만들지 마세요') > 0, true);
t('말투를 바꾸지 말라고 적는다', introP.indexOf('**말투를 바꾸지 마세요.**') > 0, true);
t('줄 차례를 맨 뒤에 붙인다',
  introP.lastIndexOf('지켜야 할 줄 차례') > introP.lastIndexOf('════════ 주제 ════════'), true);
/* ⚠️ 벤치마크의 네 줄 순서가 본인 틀을 이겼다 — 거기에 단서를 붙였다 */
t('벤치마크에 단서를 붙인다', introP.indexOf('본인이 쓰던 틀이 위다') > 0, true);
/* 틀이 없으면 이 덩어리도 없다 */
t('틀이 없으면 안 붙인다',
  P6.buildPrompt('재물운', {}).indexOf('마지막으로 — 제일 중요합니다') > 0, false);

/* 파이프라인이 그 검사를 쓰는가 */
t('인사글도 틀을 확인한다', pipeSrc3.indexOf("o.form && o.form.id === 'intro'") > 0, true);
t('내 인사글 틀로 잰다', pipeSrc3.indexOf('dailyshape.check(introTpl,') > 0, true);
